...
def check_book():
    ...
    conn.close()
    if existing_book and data['book_title'].strip() != '' :
...
def add_book():
    ...
    """
    data = request.get_json()
    # check whether book name is empty
    if data['book_title'].strip() == '':
        return jsonify({"message": "書名不得空白！"}), 200
